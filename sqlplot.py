...
def multiplot(sqlbuffer, multiplot_columns):
	...
	# print(group_query)
	group_query = re.sub('MULTIPLOT', ','.join(map(lambda col: '"%s"' % col, multiplot_columns)), sqlbuffer)
...
try:
	with open('pgf_color_entries.txt','r') as txtfile:
		for line in txtfile.readlines():
			...
			if len(cols) == 2:
				color_entries[cols[0]] = int(cols[1])
except IOError:
			...
			if texLine.startswith(filetype.comment()):
				...
				if outfiletype == Filetype.CSV:
					...
					for entry_id in range(len(entrynames)):
						...
						print('\\addlegendentry{%s};' % str(entry), file=outfile)
